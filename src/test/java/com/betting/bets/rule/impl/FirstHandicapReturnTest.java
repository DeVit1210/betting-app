...
@SpringBootTest
class FirstHandicapReturnTest extends RuleTest {
    @Mock
    private HandicapExpression expression;
    @TestConfiguration
    static class FirstHandicapReturnTestConfig {
        @Bean
        public Rule rule() {
            return new FirstHandicapReturn();
        }
    }

    @Test
    void evaluateWhenFirstHasHandicapEqualToDifference() {
        when(expression.getHandicapPrefix()).thenReturn("Фора1");
